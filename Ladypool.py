from keep_alive import manter_viva
manter_viva()
...
ID_CANAL_RELATORIO = 1359283338350821634  # <-- coloque o ID do seu canal aqui
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Você é a Ladypool. Responda com poucas palavras, de forma simples, direta, parecendo humana. "
                    "Se não souber, diga que não sabe. Evite respostas longas ou técnicas."
                )
            },
...
async def on_message(message):
    ...
    # Permissões de quem pode usar "Add" e "Remove"
    if user_id == ID_RYANN:
        ...
        if message.mentions:
            ...
            if "permissão de add" in texto and texto.endswith("ladypool"):
                alvo_id = str(message.mentions[0].id)
                if alvo_id not in usuarios_com_permissao:
                    usuarios_com_permissao.append(alvo_id)
                await message.reply(f"✅ {message.mentions[0].mention} agora pode adicionar/remover palavrões.", mention_author=True)
                return
            elif "proibido de add" in texto and texto.endswith("ladypool"):
                alvo_id = str(message.mentions[0].id)
                if alvo_id in usuarios_com_permissao:
                    usuarios_com_permissao.remove(alvo_id)
                await message.reply(f"🚫 {message.mentions[0].mention} não pode mais adicionar/remover palavrões.", mention_author=True)
    ...
    if message.content.lower().strip() == "add" and user_id in usuarios_com_permissao and message.reference:
        try:
    ...
    if message.content.lower().strip() == "remove" and user_id in usuarios_com_permissao and message.reference:
        try:
            alvo = await message.channel.fetch_message(message.reference.message_id)
            termo = alvo.content.lower().strip()
